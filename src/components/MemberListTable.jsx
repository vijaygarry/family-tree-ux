...
export const MemberListTable = ({ membersList, familyNameInHindi }) => {
  ...
  const relatedCount = membersList.filter((m) => m?.belongsToSameFamily === false).length;
  ...
  return (
    <div className="mb-4 mt-5">
        ...
        <table className="table table-bordered table-striped family-members">
          ...
          <tbody>
            {membersList.map((member) => (
              member?.belongsToSameFamily && (
                <DisplayMemberRow member={member} /> )
            ))}
          </tbody>
        </table>
      </div>
      ...
      {relatedCount > 0 && (
        <>
          <h5 className="mb-4">Related Members </h5>
          <div className="table-responsive">
            <table className="table table-bordered table-striped family-members">
              <MemberTableHeading />
              <tbody>
                {membersList.map((member) => (
                  member?.belongsToSameFamily === false && (
                    <DisplayMemberRow member={member} /> )
                ))}
              </tbody>
            </table>
